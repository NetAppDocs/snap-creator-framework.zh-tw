...
預設情況下，如果一個或多個資料庫處於不一致或損壞狀態，備份作業將失敗。您可以依照步驟 link:task_using_the_gui_to_create_a_configuration_file.html["使用GUI建立組態檔的工作"] 中的說明，強制外掛程式在發生錯誤的情況下繼續執行備份作業。
...
Domino外掛程式無法搭配snap_timestamp_only = N使用Snap_timestamp_only應設為Y
 a| 
如需更多資訊，請參閱步驟 link:task_using_the_gui_to_create_a_configuration_file.html["使用 GUI 建立組態檔"]。
